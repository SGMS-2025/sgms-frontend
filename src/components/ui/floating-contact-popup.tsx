...
const FloatingContactPopup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    alert(t('contact_popup.success_message'));
  };
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label={t('contact_popup.open_form')}
      >
      ...
      {isOpen && (
        ...
        >
          ...
          <button
            ...
            onClick={handleClose}
            aria-label={t('contact_popup.close')}
          />
          <div
          ...
          >
            ...
            <div className="mb-6">
              ...
              <p className="mt-2 text-sm text-slate-500">{t('contact_popup.description')}</p>
            </div>
            ...
            <div className="mb-6 space-y-3">
              <div className="flex items-center gap-3 rounded-xl bg-orange-50 p-3">
                ...
                <div>
                  <p className="text-xs font-medium text-slate-600">{t('contact_popup.hotline_label')}</p>
                  <p className="text-sm font-semibold text-orange-600">{t('contact_popup.hotline_number')}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-xl bg-orange-50 p-3">
                <Mail className="h-4 w-4 text-orange-600" />
                <div>
                  <p className="text-xs font-medium text-slate-600">{t('contact_popup.email_label')}</p>
                  <p className="text-sm font-semibold text-orange-600">{t('contact_popup.email_address')}</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <Input
                placeholder={t('contact_popup.name_placeholder')}
                className="h-12 rounded-xl border-slate-200"
                required
              />
              <Input
                placeholder={t('contact_popup.email_placeholder')}
                type="email"
                className="h-12 rounded-xl border-slate-200"
                required
              />
              <Input
                placeholder={t('contact_popup.phone_placeholder')}
                className="h-12 rounded-xl border-slate-200"
                required
              />
              <Button
              ...
              >
                {isSubmitting ? (
                  t('contact_popup.submitting')
                ) : (
                  <>
                    {t('contact_popup.submit_button')}
                    <ArrowRight className="ml-2 h-4 w-4" />
                  </>
                )}
              </Button>
              <p className="text-center text-xs text-slate-500">
                {t('contact_popup.privacy_text')}{' '}
                <a href="/privacy-policy" className="text-orange-600 hover:underline">
                  {t('contact_popup.privacy_link')}
                </a>{' '}
                {t('contact_popup.privacy_suffix')}
              </p>
            </form>
          </div>
        </div>
