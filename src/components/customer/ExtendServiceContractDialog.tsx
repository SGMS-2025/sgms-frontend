...
interface ExtendServiceContractDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  contractId: string;
...
export const ExtendServiceContractDialog: React.FC<ExtendServiceContractDialogProps> = ({
  isOpen,
  onClose,
  onSuccess,
  contractId,
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.extensionMonths > 12) {
      newErrors.extensionMonths = 'Thời gian gia hạn không được vượt quá 12 tháng';
    }
  ...
  const handleSubmit = () => {
    ...
    serviceContractApi
      .extendServiceContract(contractId, {
        extensionMonths: formData.extensionMonths,
        notes: formData.notes
      })
      .then(() => {
        toast.success(`Gia hạn gói ${contractType === 'PT' ? 'PT' : 'lớp học'} thành công!`, {
      ...
      .catch((error) => {
        console.error('Error extending service contract:', error);
      ...
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Gia hạn gói {contractType === 'PT' ? 'PT 1-1' : 'lớp học'}
          </DialogTitle>
        </DialogHeader>
