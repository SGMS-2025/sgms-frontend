...
/**
 * ============================================
 * EnrolledStudentsList Component
 * ============================================
 *
 * Displays list of enrolled students in a class.
 * Shows:
 * - Student name, email
 * - Service package name (must match class package)
 * - Enrollment date
 * - Session usage (if available from contract)
 * - Status (ACTIVE/INACTIVE)
 *
 * Features:
 * - Search by student name
 * - Filter by status
 * - Shows only students with matching service package
 */
    ...
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-red-600 font-medium">Error loading students</p>
      ...
      {showHeader && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">Enrolled Students ({validStudents.length})</h3>
          </div>
      ...
      {!compact && classServicePackageName && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <p className="text-sm text-blue-900">
            <span className="font-semibold">Showing students with package:</span> {classServicePackageName}
          </p>
          {validStudents.length < filteredStudents.length && (
            <p className="text-xs text-blue-700 mt-1">
              {filteredStudents.length - validStudents.length} student(s) filtered out (different package)
            </p>
          )}
        </div>
      )}
      ...
      {validStudents.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-lg font-medium">
            {search || statusFilter !== 'ALL' ? 'No students found' : 'No enrolled students'}
          </p>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-600">
                    ...
                    {sessionsInfo && (
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          Sessions: {sessionsInfo.used}/{sessionsInfo.total} ({sessionsInfo.remaining} remaining)
                        </span>
                      </div>
                    )}
                  </div>
