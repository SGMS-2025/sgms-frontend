...
/**
 * ============================================
 * ClassListView Component
 * ============================================
 *
 * Full management view for classes.
 * Features:
 * - List all classes with pagination
 * - Card/Table view toggle
 * - Search by name
 * - Filter by status and trainer
 * - Edit, Enroll, Delete buttons
 * - Create new class button
 * - Auto-refetch after CRUD operations
 */
...
export const ClassListView: React.FC<ClassListViewProps> = ({ branchId: propBranchId }) => {
  ...
  React.useEffect(() => {
    if (classes.length > 0) {
      console.log('[ClassListView] Classes loaded:', classes.length, 'branchId:', branchId);
    }
  }, [classes, branchId]);
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">{classes.length > 0 ? `${classes.length} class(es)` : 'No classes'}</p>
          </div>
          <Button onClick={() => setFormModal({ open: true })} className="bg-orange-500 hover:bg-orange-600 text-white">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {error && !loading && (
          <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm text-red-600 font-medium">Error loading classes</p>
        ...
        {!loading && !error && classes.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <Button onClick={() => setFormModal({ open: true })} className="mt-4">
              <Plus className="w-4 h-4 mr-2" />
              Create Class
            </Button>
          </div>
