...
/**
 * ============================================
 * ClassInfoCard Component
 * ============================================
 *
 * Displays class information in a compact card format.
 * Shows:
 ...
 * - isCompact: Show minimal info (for use in modals)
 */
...
export const ClassInfoCard: React.FC<ClassInfoCardProps> = ({
  ...
  const moreTrainers = classData.trainerIds.length > 2 ? `+${classData.trainerIds.length - 2} more` : '';

  // Format schedule
  const scheduleDisplay = classData.schedulePattern.daysOfWeek
    .slice(0, 3)
    .map((day) => DAY_LABELS[day]?.substring(0, 3))
    .join(', ');

  const moreSchedule =
    classData.schedulePattern.daysOfWeek.length > 3 ? `+${classData.schedulePattern.daysOfWeek.length - 3} days` : '';
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              <p className="text-xs text-gray-500 line-clamp-1">
                {typeof classData.servicePackageId === 'object' ? classData.servicePackageId.name : 'Service Package'}
              </p>
              <Badge
